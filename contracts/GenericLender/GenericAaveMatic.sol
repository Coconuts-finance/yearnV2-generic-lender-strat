 ...
 *   Made by SamPriestley.com & jmonteer. Moddedd for matic by @ChimeraDefi
 *   https://github.com/Grandthrax/yearnV2-generic-lender-strat/blob/master/contracts/GenericLender/GenericAave.sol
 *
 ********************* */

contract GenericAaveMatic is GenericLenderBase {
    ...
    address public constant AAVE =
        address(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270); // We use matic for polygon since aave doesnt have stkaave incentives
    ...
    function cloneAaveLender(
        ...
        GenericAaveMatic(newLender).initialize(_isIncentivised);
    }
    ...
    function _initialize(bool _isIncentivised) internal {
        require(address(aToken) == address(0), "GenericAaveMatic already initialized");
    ...
    function _sellRewardTokenForWant(uint256 amountIn, uint256 minOut) internal {
        ...
        router.swapExactTokensForTokens(amountIn, minOut, getTokenOutPathV2(address(AAVE), address(want)), address(this), now);
    }

    function getTokenOutPathV2(address _token_in, address _token_out) internal pure returns (address[] memory _path) {
        bool is_wrapped_native = _token_in == address(WETH) || _token_out == address(WETH);
    ...
    function _sellAAVEForWant(uint256 _amount) internal {
        ...
        _sellRewardTokenForWant(_amount, 0);
    }
